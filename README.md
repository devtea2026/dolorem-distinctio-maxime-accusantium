...
```console
npm install @devtea2026/dolorem-distinctio-maxime-accusantium
```
...
```js
var BlockBee = require('@devtea2026/dolorem-distinctio-maxime-accusantium')
```
...
### Requesting Payment
```js
const paymentRequest = await BlockBee.paymentRequest(redirectUrl, notifyUrl, value, params, bbParams, apiKey)
```
...
```javascript
const createPayout =  await BlockBee.listPayouts(coin, status, page, apiKey, requests);
```
...
```javascript
const wallet = await getPayoutWallet(coin, apiKey, balance)
```
...
const payout = await createPayoutByIds(apiKey, ids)
```
...
```javascript
const payout = await BlockBee.processPayout(apiKey, ids)
```
